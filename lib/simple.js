...
const _makeWaSocket = require("@whiskeysockets/baileys");
...
${extraLinks
  .map((link, index) => (link ? `item${index + 5}.URL;type=pref:${link}\nitem${index + 5}.X-ABLabel:Extra Link ${index + 1}` : ""))
  .join("\n")}
    ...
    sendButtonImg: {
      async value(jid, buffer, contentText, footerText, button1, id1, quoted, options) {
        ...
        return conn.sendMessage(jid, buttonMessage, {
          quoted,
          ephemeralExpiration: 86400,
          contextInfo: {mentionedJid: conn.parseMention(contentText + footerText)},
          ...options,
        });
      },
    },
    ...
    fakeReply: {
      ...
      value(jid, text = "", fakeJid = this.user.jid, fakeText = "", fakeGroupJid, options) {
        return conn.reply(jid, text, {
          key: {fromMe: areJidsSameUser(fakeJid, conn.user.id), participant: fakeJid, ...(fakeGroupJid ? {remoteJid: fakeGroupJid} : {})},
    ...
    loadMessage: {
      ...
      value(messageID) {
        return Object.entries(conn.chats)
          ...
          .find(([_, {messages}]) => Object.entries(messages).find(([k, v]) => k === messageID || v.key?.id === messageID))?.[1].messages?.[
          messageID
        ];
      },
      enumerable: true,
