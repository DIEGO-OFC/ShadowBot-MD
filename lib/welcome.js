...
const src = join(__dirname, "..", "galeria");
const _svg = readFileSync(join(galería, "welcome.svg"), "utf-8");
const barcode = (data) => {
 ...
 * @returns {string}
 */
const genSVG = async ({wid = "", pp = join(galeria, "avatar_contact.png"), title = "", name = "", text = "", background = ""} = {}) => {
  let el = {
...
const render = async (
  {
    wid = "",
    pp = toBase64(readFileSync(join(galeria, "avatar_contact.png")), "image/png"),
    name = "",
    title = "",
    text = "",
    background = toBase64(readFileSync(join(galeria, "Aesthetic", "Aesthetic_000.jpeg")), "image/jpeg"),
  } = {},
  format = "png"
) => {
