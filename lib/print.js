...
export default async function (m, conn = { user: {} }) {
  ...
  console.log(
    `╔═══════⊂(𝐃𝐎𝐑𝐑𝐀𝐓-𝐁𝐎𝐓)⊃══════════ 
 ║🔰 <span class="math-inline">\{chalk\.blue\.bold\(sender\)\}\\n║⏱️ㅤ</span>{chalk.black.cyan(m.messageTimestamp ? new Date(1000 * (m.messageTimestamp.low || m.messageTimestamp)).toTimeString() : new Date()).toTimeString()}\n║🌠ㅤ<span class="math-inline">\{chalk\.magenta\.bold\(filesize\)\} 
║🛡️ㅤ</span>{chalk.white.bold(sender)}\n║💎ㅤ<span class="math-inline">\{chalk\.yellow\.bold\(m\.exp \+ "\|" \+ \(user ? user\.dolares \: ""\) \+ "\|" \+ \(user ? user\.level \+ user\.limit \: ""\)\)\}\\n║📩ㅤ</span>{chalk.magenta.bold(m.chat + (chat ? " ~" + chat : ""))}\n║🗨️ㅤ<span class="math-inline">\{chalk\.black\.bold\(m\.mtype\.replace\(/message</span>/i, "").replace("audio", m.msg.ptt ? "PTT" : "audio").replace(/^./, (v) => v.toUpperCase()))}
 ╚═════════⊂(𝐃𝐎𝐑𝐑𝐀𝐓-𝐁𝐎𝐓)⊃════════`
 📧 mensaje/media: 
 `.trim(),
