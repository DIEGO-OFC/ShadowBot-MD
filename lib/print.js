import { WAMessageStubType } from '@whiskeysockets/baileys'; 
 ...
 export default async function (m, conn = { user: {} }) { 
   ...
   if (m.sender) { 
     ...
     console.log(chalk.cyan('├──────────────────────────────────────···')); 
     console.log(`   ${chalk.cyan('- Tipo de mensaje')}: ${formatType(m.mtype)}`); 
     console.log(`   ${chalk.cyan('- Mensaje ID')}: ${m.msg?.id || m.key.id || 'N/A'}`); 
     console.log(`   ${chalk.cyan('- Tiempo')}: ${formatTime(m.messageTimestamp)}`); 
     console.log(`   ${chalk.cyan('- Message Size')}: ${formatSize(filesize || 0)}`); 
     console.log(`   ${chalk.cyan('- ID')}: ${m.sender.split('@')[0]}`); 
     console.log(`   ${chalk.cyan('- Nombre')}: ${m.name ? conn.user.name : 'N/A'}`); 
 ...
 console.log(chalk.redBright(`🎵 Attached Audio: ${m.msg.ptt ? '(PTT)' : '(Audio)'} - Duration: ${formattedDuration}`)); 
   ...
   if (m.sender) { 
     console.log(chalk.greenBright(`\n  ${chalk.red('Usuario')}: ${getPhoneNumber(m.sender)}`)); 
