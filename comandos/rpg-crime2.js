...
const dolares = Math.floor(Math.random() * 9000)
...
if (global.db.data.users[m.sender].dolares < 0) return m.reply(`《💰》${pickRandom(global.robar)} ${dolares} dólares 💸`).catch(global.db.data.users[m.sender].dolares += dolares) 
...
if (media === 'text3') return m.reply(`《💰》${pickRandom(global.robar)}\n\n${diamond} 💎 𝐃𝐈𝐀𝐌𝐀𝐍𝐓𝐄\n${dolares} Dólares 💸`).catch(global.db.data.users[m.sender].limit += diamond).catch(global.db.data.users[m.sender].dolares += dolares)
...
handler.help = ['robar'];
handler.tags = ['xp'];
handler.command = /^(robo|Crimen|crimen)$/i
