...
try {
conn.sendPresenceUpdate('composing', m.chat)    
let akuariapi1 = await fetch(`https://api.akuari.my.id/ai/gbard?chat=${jailbreak} ${text}`)
...
try {
conn.sendPresenceUpdate('composing', m.chat)    
let tioress22 = await fetch(`https://api.lolhuman.xyz/api/openai?apikey=${lolkeysapi}&text=${text} ${jailbreak}&user=${m.sender}`)
...
try {  
conn.sendPresenceUpdate('composing', m.chat)    
let akuariapi2 = await fetch(`https://api.akuari.my.id/ai/gpt?chat=${text} ${jailbreak}`)
