let handler = async (m, {conn, text, usedPrefix, isAdmin, isOwner, participants}) => {
  ...
  let id = m.chat;
  if (id in conn.vote) await conn.sendButton(m.chat, `*Todavía hay votos en este chat.!*`, dorre, null, [["𝙱𝙾𝚁𝚁𝙰𝚁🗑️", `${usedPrefix}-vote`]], m);
  ...
  const sections = [
    {
      ...
      rows: [
        {title: "「 ✅ 」𝐕𝐎𝐓𝐎 𝐀 𝐅𝐀𝐕𝐎𝐑", rowId: `#upvote`},
        {title: "「 ❌ 」𝐕𝐎𝐓𝐎 𝐄𝐍 𝐂𝐎𝐍𝐓𝐑𝐀", rowId: `#devote`},
      ],
    },
  ];
  const listMessage = {
