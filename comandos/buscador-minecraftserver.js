...
let handler = async (m, {conn, args, usedPrefix, DevMode}) => {
  try {
    ...
    switch (type) {
      ...
      case "java":
        util
          .status(args[1]) // port is default 25565
          .then((response) => {
            console.log(response);
            m.reply(
              `*Info Server Java Edition*\n\nip/host: *${response.host}*\nPuerto: *${response.port}*\nVersion: *${response.version}*\nProtocol Version: *${response.protocolVersion}*\nJugadores conectados: *${response.onlinePlayers}*\nMax Player: *${response.maxPlayers}*\nMotd: *${response.description.descriptionText}*`
            );
          })
          .catch((error) => {
            console.error(error);
      ...
      default:
        return conn.reply(
          m.chat,
          `Formato de uso: ${usedPrefix}server <bedrock | java> <ip> <port>\nEjemplo: *${usedPrefix}server bedrock play.nethergames.org 19132*`.trim(),
          m
        );
    }
  } catch (e) {
