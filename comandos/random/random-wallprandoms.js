...
let handler = async (m, {command, conn}) => {
  let fgif = {
    ...
    message: {videoMessage: {title: `️u`, h: `Hmm`, seconds: "99999", gifPlayback: "true", caption: `🧿 𝚃𝚑𝚎 𝙼𝚢𝚜𝚝𝚒𝚌 - 𝙱𝚘𝚝 🔮`, jpegThumbnail: false}},
  };
  let apikey = keysxxx;
  ...
  if (command == "wpmontaña") {
    ...
    conn.sendButton(m.chat, `_${command}_`.trim(), `*◈•@${who.split("@s.whatsapp.net")[0]}*`, haha.data, [["🔄 𝚂𝙸𝙶𝚄𝙸𝙴𝙽𝚃𝙴 🔄", `/${command}`]], fgif, {
      mentions: [who],
    });
  }
  if (command == "pubg") {
  ...
  if (command == "wprandom") {
    ...
    let json = res.data;
    let url = json.url;
    conn.sendButton(m.chat, `_${command}_`.trim(), `*◈•@${who.split("@s.whatsapp.net")[0]}*`, url, [["Gracias💖", `/bot gracias`]], fgif, {
      mentions: [who],
    });
  }
  if (command == "coffee") {
