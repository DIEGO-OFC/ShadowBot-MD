...
let handler = async (m, { conn, args, command, jid, text, usedPrefix }) => {
  ...
  if (args[0] == '1') { // Inventario 1 
    ...
    let dolares = member.dolares
    let exp = member.exp
    let token = member.joincount
    let dia = member.limit
    let tiketm = member.healtmonster

    let sorteddolares = Object.entries(global.db.data.users).sort((a, b) => b[1].dolares - a[1].dolares)
    ...
    let usersdolares = sorteddolares.map(v => v[0])
 ...
 ┃ ${rpgg.emoticon('dolares')} *Dolares: » ${dolares}* 
 ...
 _2.Top Dolares_ *${usersdolares.indexOf(m.sender) + 1}* _de_ *${usersdolares.length}* 
 ...
 *│ 💵Trabajar${new Date - user.lastword < 600000 ? '❌' : '✅'}* 
 ...
 *╰─⋆─⋆─⋆─⋆─⋆─⋆─⋆─⋆─┄⸙*`.trim()
    ...
    conn.sendFile(m.chat, imgr, 'zirax.jpg', caption, fkontak, m, { mentions: conn.parseMention(caption) })
