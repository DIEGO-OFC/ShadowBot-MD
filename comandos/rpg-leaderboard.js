...
║ ${sortedDolares
    ...
    .map(
      ({jid, dolares}, i) =>
        `${i + 1}. ${participants.some((p) => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : "@"}${jid.split`@`[0]} *${dolares}  💸*`
    ).join`\n`}
║═════════════════
║ ➢ *TOP ${len} XP ⚡* 
...
║ ${sortedExp
    .slice(0, len)
    .map(
      ({jid, exp}, i) => `${i + 1}. ${participants.some((p) => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : "@"}${jid.split`@`[0]} *${exp} ⚡*`
    ).join`\n`}
║═════════════════
║ ➢ *TOP ${len} NIVEL 🔅* 
...
║ ${sortedLevel
    ...
    .map(
      ({jid, level}, i) =>
        `${i + 1}. ${participants.some((p) => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : "@"}${jid.split`@`[0]} *${level} 🔅*`
    ).join`\n`}
║═════════════════
║ ➢ *TOP ${len} ROL | RANGO 💪* 
║ Tú : *${usersLevel.indexOf(m.sender) + 1}* de *${usersLevel.length} Usuarios*
...
║ ${sortedLim
    ...
    .map(
      ({jid, limit}, i) =>
        `${i + 1}. ${participants.some((p) => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : "@"}${jid.split`@`[0]} *${limit} 💎*`
    ).join`\n`}
═════════════════
║ ➢ *TOP ${len} TOKENS  ☯️* 
...
║ ${sortedJoincount
    ...
    .map(
      ({jid, joincount}, i) =>
        `${i + 1}. ${participants.some((p) => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : "@"}${jid.split`@`[0]} *${joincount} ☯️*`
    ).join`\n`}
╚════ ≪ •❈• ≫ ════╝
`.trim();
