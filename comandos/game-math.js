  ...
  if (args.length < 1)
    return conn.sendHydrated(
      ...
      [
        ...
        ["𝙼𝙰𝚃𝙴𝚂 𝙷𝙰𝚁𝙳", `${usedPrefix + command} hard`],
      ],
      m
    );
  ...
  global.math[id] = [
    ...
    setTimeout(() => {
      if (global.math[id])
        conn.sendButton(
          m.chat,
          `*[❗𝐈𝐍𝐅𝐎❗] 𝚂𝙴 𝙰𝙷 𝙵𝙸𝙽𝙰𝙻𝙸𝚉𝙰𝙳𝙾 𝙴𝙻 𝚃𝙸𝙴𝙼𝙿𝙾 𝙿𝙰𝚁𝙰 𝚁𝙴𝚂𝙿𝙾𝙽𝙳𝙴𝚁*\n\n*𝙻𝙰 𝚁𝙴𝚂𝙿𝚄𝙴𝚂𝚃𝙰 𝙴𝚂 ${math.result}*`,
          author,
          null,
          [["𝚅𝙾𝙻𝚅𝙴𝚁 𝙰 𝙸𝙽𝚃𝙴𝙽𝚃𝙰𝚁", `${usedPrefix + command} ${math.mode}`]],
          global.math[id][0]
        );
