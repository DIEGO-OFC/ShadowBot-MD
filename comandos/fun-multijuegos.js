...
let handler = async (m, {conn, usedPrefix, command, usedPrefix: _p, text}) => {
  ...
  if (command == "multijuegos") {
    ...
    const templateButtonsReplyMessage = [
      {index: 1, urlButton: {displayText: "†𝐃⃟𝕺𝐑⃯𝐑𝐇⃯𝚵𝐓᪣𝕭⃯𝚹⃯𝐓⃤", url: "https://github.com/DIEGO-OFC/DORRAT-BOT-MD"}},
  ...
  if (command == "juegos") {
    ...
    let ss = await `†𝐃⃟𝕺𝐑⃯𝐑𝐇⃯𝚵𝐓᪣𝕭⃯𝚹⃯𝐓⃤ | DIEGO-OFC`;
  ...
  if (command == "juegos") {
    ...
    let juegos2 = `${pickRandom([
      `${c}ppt piedra`,
      `${c}mates noob`,
      `${c}pregunta El cielo y el infierno existen?`,
      `${c}pregunta Hay una sola vida?`,
      `${c}pregunta Los robots y androides podrán tener sentimientos en algún momento?`,
      `${c}pregunta De verdad el universo es infinito?`,
      `${c}pajera Yo`,
      `${c}manca Yo`,
      `${c}manco Yo`,
      `${c}pregunta Hay un destino?`,
      `${c}pregunta Se puede tener todo lo que se quiere?`,
    ])}`;
    let juegos3 = `${pickRandom([
      `${c}puto Él de arriba`,
      `${c}puta La de arriba`,
      `${c}pregunta Se puede viajar en el tiempo?`,
      `${c}pregunta Hay un solo Universo?`,
      `${c}rata Yo`,
      `${c}mates impossible`,
      `${c}prostituto Yo`,
      `${c}prostituta Yo`,
      `${c}pregunta Soy Pro en el Grupo?`,
      `${c}mates impossible2`,
      `${c}rata Yo`,
    ])}`;
    let emojis1 = `${pickRandom([`😃`, `😳`, `🤑`, `🤠`, `🤭`, `😇`, `😋`, `🥴`])}`;
    ...
    let sss = await `†𝐃⃟𝕺𝐑⃯𝐑𝐇⃯𝚵𝐓᪣𝕭⃯𝚹⃯𝐓⃤ | DIEGO-OFC `;
