 ...
 *🐊 ${ar6} ${anti6}*		    *🐓 ${ar12} ${anti12}*`;
  ...
  if (new Date() - global.db.data.users[m.sender].lastberburu < 2700000)
    return conn.sendButton(
      m.chat,
      `𝙿𝙾𝚁 𝙵𝙰𝚅𝙾𝚁 𝙳𝙴𝚂𝙲𝙰𝙽𝚂𝙰 𝚄𝙽 𝙼𝙾𝙼𝙴𝙽𝚃𝙾 𝙿𝙰𝚁𝙰 𝚂𝙴𝙶𝚄𝙸𝚁 𝙲𝙰𝚉𝙰𝙽𝙳𝙾`,
      `⫹⫺ 𝚃𝙸𝙴𝙼𝙿𝙾 ${clockString(time - new Date())}\n${wm}`,
      null,
      [
        ["🏞️ 𝙰𝙽𝙸𝙼𝙰𝙻𝙴𝚂 𝙲𝙰𝙿𝚃𝚄𝚁𝙰𝙳𝙾𝚂 ", ".kandang"],
        [`🎒 𝙸𝙽𝚅𝙴𝙽𝚃𝙰𝙴𝚁𝙸𝙾`, `.inventario`],
      ],
      fkontak,
      m
    );
  ...
  setTimeout(() => {
    conn.sendHydrated(m.chat, hsl, wm, null, md, `𝘿𝙤𝙧𝙧𝙖𝙩-𝘽𝙤𝙩-𝙈𝘿`, null, null, [[null, null]], null);
  }, 20000);
  ...
  setTimeout(() => {
    conn.sendHydrated(
      m.chat,
      `${conn.getName(m.sender)} *${[
        "Looking for hunting implements...",
        "Getting everything ready for the hunt!!",
        "Establishing Hunting Place...",
        "PREPARING HUNTING PLACE!!",
      ].getRandom()}*`,
      wm,
      null,
      null,
