...
*${usedPrefix + command} ${lenguajeGB.lenguaje() == "en" ? "close" : "cerrar"} 1*`;
  ...
  await conn.groupSettingUpdate(m.chat, isClose).then(async (_) => {
    m.reply(
      `${lenguajeGB["smsAvisoRG"]()}${lenguajeGB["smsGrupoTime3"]()} ${
        isClose == "announcement" ? lenguajeGB.smsGrupoTime4() : lenguajeGB.smsGrupoTime5()
      } ${args[1] ? `${lenguajeGB["smsGrupoTime6"]()} ${clockString(timeoutset)}*` : ""}`
    );
  });
  if (args[1]) {
    setTimeout(async () => {
