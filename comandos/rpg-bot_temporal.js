  ...
  if (!(isPrems || isOwner || isROwner)) {
    ...
    await conn.sendButton(m.chat, texto7, `${await conn.getName(res)} | ` + wm, null, [[`🍀 𝙈 𝙀 𝙉 𝙐`, `.menu`]], fkontak, m).then(async () => {
      ...
      await conn.sendButton(
        ...
        null,
        [
          [`+30 𝙈𝙄𝙉𝙐𝙏𝙊𝙎 𝙀𝙉 𝙂𝙍𝙐𝙋𝙊`, `${usedPrefix + command} ${args[0]} 3`],
          [`🍀 𝙈 𝙀 𝙉 𝙐`, `.menu`],
        ],
        fkontak,
        m
      ...
      await conn.sendButton(
        m.chat,
        botdate,
        `${igfg} 𝙀𝙎 𝙐𝙉 𝘽𝙊𝙏 𝘿𝙀 𝙒𝙃𝘼𝙏𝙎𝘼𝙋𝙋 𝙌𝙐𝙀 𝙏𝙀 𝘼𝙔𝙐𝘿𝘼𝙍𝘼 𝙍𝙀𝘼𝙇𝙄𝙕𝘼𝙍 𝘿𝙄𝙁𝙀𝙍𝙀𝙉𝙏𝙀𝙎 𝘼𝘾𝙏𝙄𝙑𝙄𝘿𝘼𝘿𝙀𝙎 🪄 𝘼𝙇 𝙋𝙍𝙄𝙑𝘼𝘿𝙊 𝙊 𝙂𝙍𝙐𝙋𝙊 𝙔 𝙏𝘼𝙈𝘽𝙄𝙀𝙉 𝙏𝙀 𝙑𝘼𝙎 𝘼 𝘿𝙄𝙑𝙀𝙍𝙏𝙄𝙍 🎈 𝘾𝙊𝙉 𝙎𝙐𝙎 𝙈𝙐𝙇𝙏𝙄𝙋𝙇𝙀𝙎 𝙁𝙐𝙉𝘾𝙄𝙊𝙉𝙀𝙎, 𝘿𝙄𝙎𝙁𝙍𝙐𝙏𝘼 𝘿𝙀 𝘿𝙊𝙍𝙍𝘼𝙏𝘽𝙊𝙏!!!  👑\n\𝙣🌠 𝘿𝙊𝙍𝙍𝘼𝙏𝘽𝙊𝙏 𝙁𝙐𝙀 𝙄𝙉𝙑𝙄𝙏𝘼𝘿𝙊 𝙋𝙊𝙍:\n*${username}*`,
        gata.getRandom(),
    ...
    let res = await conn.groupAcceptInvite(code);
    conn.reply(m.chat, `✅ *SE HA UNIDO DORRATBOT AL GRUPO!!!*`, m).then(async () => {
      ...
      await conn.reply(
        m.chat,
        `${wm} *ES UN BOT DE WHATSAPP QUE TE AYUDARÁ A REALIZAR DIFERENTES ACTIVIDADES 🪄 AL PRIVADO O GRUPO Y TAMBIÉN TE VAS A DIVERTIR 🎈 CON SUS MULTIPLES FUNCIONES, DISFRUTA DE DORRATBOT!!! 😎*\n\n👑 *DORRATBOT FUE INVITADO POR:*\n${username}`,
        m
      );
