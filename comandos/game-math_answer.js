...
let handler = async (m, {conn}) => {
  ...
  if (!(m.chat in global.math))
    return conn.sendButton(m.chat, "*[❗𝐈𝐍𝐅𝐎❗] 𝚈𝙰 𝚂𝙴 𝙰𝙷 𝚁𝙴𝚂𝙿𝙾𝙽𝙳𝙸𝙳𝙾 𝙰 𝙴𝚂𝙰 𝙿𝚁𝙴𝙶𝚄𝙽𝚃𝙰*", author, null, [["𝚅𝙾𝙻𝚅𝙴𝚁 𝙰 𝙹𝚄𝙶𝙰𝚁", "/mates"]], m);
  if (m.quoted.id == global.math[id][0].id) {
    ...
    if (m.text == math.result) {
      ...
      if (--global.math[id][2] == 0) {
        conn.sendButton(
          m.chat,
          `*𝚂𝙴 𝙰𝙲𝙰𝙱𝙰𝚁𝙾𝙽 𝚃𝚄𝚂 𝙾𝙿𝙾𝚁𝚃𝚄𝙽𝙸𝙳𝙰𝙳𝙴𝚂*\n*𝙻𝙰 𝚁𝙴𝚂𝙿𝚄𝙴𝚂𝚃𝙰 𝙴𝚂: ${math.result}*`,
          author,
          null,
          [["𝚅𝙾𝙻𝚅𝙴𝚁 𝙰 𝙹𝚄𝙶𝙰𝚁", `/math ${math.mode}`]],
          m
        );
        clearTimeout(global.math[id][3]);
        delete global.math[id];
