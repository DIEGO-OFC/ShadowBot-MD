...
let handler = async (m, { conn, args, isPrems, isOwner }) => {
  ...
  if (args.length === 0) { m.reply(`*Ingresa un enlace de YouTube.*`); return; }
  try {
      let urlYt = args[0];
      if (!urlYt.startsWith("http")) { m.reply(`Ingresa un link de youtube`); return; }
...
handler.command = ['ytdocumentotest']
handler.exp = 3
